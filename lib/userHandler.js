...
userHandler.lendBook = async (data, callback) => {
  if (data.method === "post") {
    isUser = await fileUtil.exist("users", data.query.name);
    isBook = await fileUtil.exist("books", data.query.book);

    //only if the book and user is available
    if (isUser && isBook) {
      fileUtil.updateAvailable("books", data.query, (err) => {
        if (!err) {
    ...
    callback(405, { message: "not foundd" });
  }
